...
class TestPlanner(TestBase):
    ...
    def setUp(self):
        ...
        self.test_ab_response = Ability(ability_id='30', tactic='response', technique_id='3', technique='3',
                                        name='response1', test='Mw==', description='response1', cleanup='', executor='sh',
                                        platform='linux', parsers=[], requirements=[self.requirement_paw_provenance],
                                        privilege=None, variations=[])
    ...
    def test_execute_no_facts(self):
        ...
        self.assertEqual(2, len(self.operation.chain))
        self.assertEqual(self.operation.chain[0].ability.ability_id, '10')
        self.assertEqual(self.operation.chain[1].ability.ability_id, '10')
    ...
    def test_execute_hunt_fact(self):
        ...
        self.run_async(self.response_planner.execute(phase=1))
        self.assertEqual(4, len(self.operation.chain))
        link_ability_ids = []
    ...
    def test_execute_detect_fact(self):
        ...
        self.run_async(self.response_planner.execute(phase=1))
        self.assertEqual(3, len(self.operation.chain))
        link_ability_ids = []
        for link in self.operation.chain:
            link_ability_ids.append(link.ability.ability_id)
        self.assertEqual(2, link_ability_ids.count('10'))
    ...
    def test_execute_all_together(self):
        ...
        self.run_async(self.response_planner.execute(phase=1))
        self.assertEqual(5, len(self.operation.chain))
        link_ability_ids = []
        for link in self.operation.chain:
            link_ability_ids.append(link.ability.ability_id)
        self.assertEqual(2, link_ability_ids.count('10'))
